...
(defun zhongwen-translate ()
  ...
  (let ((contents (if (use-region-p)
    ...
    (gptel-request contents
      :system "Write the character in pinyin and give a very short translation it into English.
   If it is an English word, give a Chinese translation instead.
   Give a structured output that looks like this:
  Character: X
  Pinyin:    X
  Meaning:   X"
