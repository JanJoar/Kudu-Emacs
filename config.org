...
(use-package mu4e
  ...
  (defun extract-email-username (email)
    (and (string-match "^\\([^@]+\\)@" email)
         (match-string 1 email)))
  
  (setq mu4e-drafts-folder (concat "/"
                            (extract-email-username user-mail-address)
                            "/Drafts")
  ...
  (setf mu4e-contexts
        (list
         (make-mu4e-context
          ...
          (lambda (msg)
            (when msg
              (string-prefix-p "/joarxpablo" (mu4e-message-field msg :maildir))))
          :vars '((user-mail-address . "joarxpablo@vonarndt.se")
                  (user-full-name    . "Joar von Arndt")
                  (smtpmail-smtp-server . "smtp.purelymail.com")
                  (smtpmail-smtp-service . "465")
                  (smtpmail-stream-type . ssl)
                  (mu4e-drafts-folder  . "/joarxpablo/Drafts")
                  (mu4e-sent-folder  . "/joarxpablo/Sent")
                  (mu4e-refile-folder  . "/joarxpablo/Archive")
                  (mu4e-trash-folder  . "/joarxpablo/Trash")))
         (make-mu4e-context
          :name "maskin"
          :match-func
          (lambda (msg)
            (when msg
              (string-prefix-p "/maskin" (mu4e-message-field msg :maildir))))
          :vars '((user-mail-address . "maskin@vonarndt.se")
                  (user-full-name    . "Joar von Arndt")
                  (smtpmail-smtp-server . "smtp.purelymail.com")
                  (smtpmail-smtp-service . "465")
                  (smtpmail-stream-type . ssl)))))
