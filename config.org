...
*** Prescient
Shows those completion results that are hopefullt most useful, both in the minibuffer and the main buffer.
...
** PDF-tools
The default "docview" mode of viewing pdfs is quite bad, and is improved immensely by the pdf-tools package. For some this may not be enough, and it is possible to replace it with an external pdf viewer (like evince or zathura).
